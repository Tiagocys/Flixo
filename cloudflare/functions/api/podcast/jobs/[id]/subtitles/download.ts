...
export const onRequestPost: PagesFunction<WorkerEnv> = async ({ params, request, env }) => {
  ...
  const url = moneyPrinterUrl(env, request, `/podcast/jobs/${encodeURIComponent(id)}/subtitles/download`);
  ...
  const response = await fetch(url, {
    ...
    headers: {
      ...
      ...(env.MONEYPRINTER_API_TOKEN ? { Authorization: `Bearer ${env.MONEYPRINTER_API_TOKEN}` } : {}),
